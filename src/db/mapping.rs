...
impl MappingStore {
    ...
    pub async fn store_mapping(&self, mapping: TaskMapping) -> Result<()> {
        ...
        debug!("Stored mapping: {} -> {:?} (status: {:?})", mapping.linear_issue_id, mapping.motion_task_id, mapping.status);
    ...
    pub async fn create_pending_mapping(&self, sync_source: &str, issue: &crate::clients::linear::LinearIssue) -> Result<TaskMapping> {
    ...
    pub async fn mark_synced(&self, sync_source: &str, linear_issue_id: &str, motion_task_id: String) -> Result<()> {
        if let Some(mut mapping) = self.get_mapping_by_linear_id(sync_source, linear_issue_id).await? {
    ...
    pub async fn list_mappings_by_status(&self, status: MappingStatus) -> Result<Vec<TaskMapping>> {
        ...
        debug!("Found {} mappings with status: {:?}", mappings.len(), status);
        Ok(mappings)
    }
